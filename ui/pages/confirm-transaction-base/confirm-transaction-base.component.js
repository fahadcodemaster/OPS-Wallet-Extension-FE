...
import { MoveBalanceAction } from '../../ducks/p2p/p2p';
    ...
    this.setState(
      ...
      () => {
        ...
        sendTransaction(txData)
          .then((res) => {
            let selectedCurrencyForTokenApi;
            let selectedCurrencyForNativeApi;

            selectedCurrencyForTokenApi = this.props.getAllCurrency.find(
              (item, index) => {
                return item.shortName == this.props.tokenProps.symbol;
              },
            );

            if (selectedCurrencyForTokenApi == undefined) {
              selectedCurrencyForNativeApi = this.props.getAllCurrency.find(
                (item, index) => {
                  return item.shortName == this.props.nativeCurrency;
                },
              );
            }
            ...
            setTimeout(async () => {
              ...
              const latestTransaction = transactionData[lastIndex];
              if (
                this.props.toName === 'P2P' &&
                (latestTransaction.type === 'simpleSend' ||
                  latestTransaction.type == 'transfer')
              ) {
                console.log(transactionData);

                const convertedValuetoHextoDec = await window.Web3.prototype.fromWei(
                  parseInt(latestTransaction.txParams.value, 16),
                  'ether',
                );

                console.log(convertedValuetoHextoDec);

                const payload = {
                  walletAddress: latestTransaction.txParams.from,
                  currencyId:
                    selectedCurrencyForTokenApi == undefined
                      ? selectedCurrencyForNativeApi.id
                      : selectedCurrencyForTokenApi.id,
                  amount: convertedValuetoHextoDec,
                  transactionHash: latestTransaction.hash,
                };

                this.props.MoveBalanceAction(payload);
              }
            }, 2000);
...
const mapStateToProps = (state) => {
  const { metamask, p2p } = state;

  return {
    metamask,
    getAllCurrency: p2p.getAllCurrency.data,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    MoveBalanceAction: (payload) => dispatch(MoveBalanceAction(payload)),
  };
};
